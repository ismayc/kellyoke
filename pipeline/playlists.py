    ...
    for r in rows:
        for p in r["perfs"]:
            w.writerow([
                ...
                p.get("orig_year") or "", p.get("year_src") or "",
                "; ".join(p.get("writers") or []), p.get("album") or "",
                p.get("orig_seconds") or "",
                p.get("age_at_cover") if p.get("age_at_cover") is not None else "",
