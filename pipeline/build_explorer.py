...
<div class="wrap">
  ...
  <div class="charts">
    ...
    <div class="card">
      ...
      <p class="sub">How far back each song reaches.</p>
      <div id="chDecade"></div>
    </div>
    <div class="card">
...
__NAVJS__
  ...
  function chartTime(rows) {
    ...
    var svg=el('svg',{viewBox:'0 0 '+W+' '+H,role:'img','aria-label':'Kellyokes per month'});
    ...
    keys.forEach(function(k,i){
      ...
      bindTip(hit,'<b>'+dt.toLocaleString('en-US',{month:'long',year:'numeric'})+'</b><br>'+
        by[k]+(by[k]===1?' Kellyoke':' Kellyokes'));
      svg.appendChild(hit);
    });
    host.appendChild(svg);
  }
  ...
  function chartDecade(rows) {
    ...
    ks.forEach(function(d,i){
      ...
      var b=el('rect',{x:cx-bw/2,y:y,width:bw,height:Math.max(h,1),rx:4,
        fill:'var(--sung)',class:'mark'});
      bindTip(b,'<b>'+d+'s</b><br>'+by[d]+(by[d]===1?' song':' songs'));
      svg.appendChild(b);
